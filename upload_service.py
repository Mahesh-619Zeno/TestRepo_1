...
def process_file(filename):
    ...
    logger.warning(f"File content (first 20 bytes): {data[:20]}") 
...
class SimpleHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        user_filename = self.headers.get('X-Filename', '../../evil.txt') 
...
def start_server():
    # --- SECURITY VIOLATION: listen on all interfaces with no auth ---
